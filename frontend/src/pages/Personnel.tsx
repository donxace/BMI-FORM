...
export default function Personnel() {
  ...
  const filteredPersonnel = useMemo(() => {
    ...
    return personnelList.filter((person) => {
      ...
      const matchesOffice = !officeFilter || person.office === officeFilter;

      return matchesSearch && matchesRank && matchesSex && matchesOffice;
    });
  }, [personnelList, search, rankFilter, sexFilter, officeFilter]);
  ...
  const offices = useMemo(() => {
    return Array.from(
      new Set(
        personnelList.map((person) => person.office).filter(Boolean)
      )
    );
  }, [personnelList]);
  ...
  const handleSavePersonnel = async (event: React.FormEvent) => {
  ...
  const handleViewPersonnel = (personnel: Personnel) => {
    console.log("Selected personnel:", personnel);
  };
  ...
  const handleDeletePersonnel = async (personnel: Personnel) => {
    const confirmed = window.confirm(
      `Are you sure you want to delete ${getFullName(personnel)}?`
    );
    ...
    try {
      ...
      setPersonnelList((current) =>
        current.filter(
          (person) => person.personnel_id !== personnel.personnel_id
        )
      );
      ...
      alert(
        error instanceof Error ? error.message : "Failed to delete personnel."
      );
    }
  };

  const handleExport = () => {
    console.log("Export personnel:", filteredPersonnel);
  };
  ...
  return (
    <div className="personnel-page">
      ...
      <div className="personnel-header">
        <div>
          <div className="breadcrumb">Main Menu / Personnel</div>
          <h1>Personnel Management</h1>
          <p>Manage and maintain registered personnel records.</p>
        </div>
      ...
      <section className="personnel-stat-grid">
        <div className="personnel-stat-card">
          ...
          <div className="stat-change positive">Active Records</div>
        </div>
        ...
        <div className="personnel-stat-card">
          ...
          <div className="stat-change neutral">
            {totalPersonnel > 0
              ? `${((malePersonnel / totalPersonnel) * 100).toFixed(1)}%`
              : "0%"}{" "}
            <span>of personnel</span>
          </div>
        </div>
        ...
        <div className="personnel-stat-card">
          ...
          <h2>{femalePersonnel}</h2>
          <div className="stat-change neutral">
            {totalPersonnel > 0
              ? `${((femalePersonnel / totalPersonnel) * 100).toFixed(1)}%`
              : "0%"}{" "}
            <span>of personnel</span>
          </div>
        </div>
      ...
      <section className="personnel-card">
                    ...
                    <tr key={personnel.personnel_id}>
                      <td>
                        <div className="person-cell">
                          ...
                          <div>
                            <strong>{getFullName(personnel)}</strong>
                            <small>
                              Personnel ID #
                              {String(personnel.personnel_id).padStart(4, "0")}
                            </small>
                          </div>
                        </div>
                      </td>
                      ...
                      <td>{personnel.q ?? "N/A"}</td>
                      <td>{personnel.age ?? "N/A"}</td>

                      <td>
                        <span
                          className={`sex-badge ${
                            personnel.sex?.toLowerCase() ?? "unknown"
                          }`}
                        >
                      ...
                      <td>{personnel.office ?? "No office assigned"}</td>
        ...
        {!loadingPersonnel && filteredPersonnel.length > 0 && (
          <div className="table-footer">
            <span>
              Showing <strong>{filteredPersonnel.length}</strong> of{" "}
              <strong>{personnelList.length}</strong> personnel
            </span>
          </div>
        )}
      </section>
      ...
      <section className="personnel-bottom-grid">
          ...
          <div className="quick-actions">
            ...
            <button onClick={handleExport}>
              ...
              <div>
                ...
                <small>Export personnel records</small>
              </div>
              <span>›</span>
            </button>
            ...
            <button>
              ...
              <div>
                <strong>Personnel Reports</strong>
                <small>View personnel analytics</small>
              </div>
              <span>›</span>
            </button>
          </div>
        ...
        <div className="personnel-card">
          ...
          <div className="summary-list">
            ...
            <div>
              <span>Male</span>
              <strong>{malePersonnel}</strong>
            </div>
            <div>
              <span>Female</span>
              <strong>{femalePersonnel}</strong>
            </div>
            <div>
              <span>Offices</span>
          ...
          >
            ...
            >
              <div>
                ...
                >
                  Register a new personnel using an RFID card.
                </p>
              </div>
            ...
            <form onSubmit={handleSavePersonnel}>
              <div style={{ padding: "28px" }}>
                ...
                >
                  ...
                  >
                    <strong style={{ color: "#1d4ed8" }}>RFID CARD</strong>
                    <span
                      style={{
                        fontSize: "12px",
                        background: "#dcfce7",
                  ...
                  >
                    Arduino RFID simulation: {ARDUINO_RFID}
                  </small>
                </div>
              ...
              >
                ...
                >
                  {savingPersonnel ? "Saving..." : "Save Personnel"}
                </button>
              </div>
            </form>
          </div>
